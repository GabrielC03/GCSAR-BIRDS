...
#| fig-cap: #| fig-cap: Heatmap showing the relationship between bird species richness and impervious surface coverage on college campuses in the US. Institutions are limited to Maine, Pennsylvania, North Carolina, and Florida. Lighter blue represents a higher concentration of data points in that area while darker blue represents a lower concentration.
# code written by Gabriel Cabrera and Lilah Shtino
...
There is no strong correlation between bird species richness and impervious surface. Performing a logarithmic transformation on mean species richness, mean impervious, or both doesn’t affect the relationship significantly or strengthen the correlation.

```{r}
#| label: log_transform_x
#| fig-cap: Heatmap of species richness vs. impervious surface with logarithmic transformations of impervious surface.
# code written by Jaela Nguyen

ggplot(ebirds_ipeds_impervious, aes(x = mean_impervious, y = species_richness)) +
  geom_bin2d() +
  labs(
    title = "log10(Impervious Surfaces)",
    x = "Impervious Surfaces",
    y = "Species Richness"
  ) +
  scale_x_log10() +
...
```
```{r}
#| label: log_transform_y
#| fig-cap: Heatmap of species richness vs. impervious surface with logarithmic transformations of species richness.
# code written by Jaela Nguyen
ggplot(ebirds_ipeds_impervious, aes(x = mean_impervious, y = species_richness)) +
  geom_bin2d() +
  labs(
    title = "log10(Species Richness)",
    x = "Impervious Surfaces",
    y = "Species Richness"
  ) +
  scale_y_log10() +
...
```
```{r}
#| label: log_transform_x_y
#| fig-cap: Heatmap of species richness vs. impervious surface with logarithmic transformations of impervious surface and species richness.
# code written by Jaela Nguyen

ggplot(ebirds_ipeds_impervious, aes(y = species_richness, x = mean_impervious)) +
  geom_bin2d() +
  labs(
    title = "log10(Impervious Surfaces), log10(Species Richness)",
    x = "Impervious Surfaces",
    y = "Species Richness"
  ) +
  scale_x_log10() +
...
#| fig-cap: Data points represent individual institutions, color-coded by the size of the student population. Smoothed trend lines illustrate how biodiversity varies with increasing impervious surface area for each institution size category.
#| # code written by Lilah Shtino
...
The Carnegie Classification system identifies colleges in the U.S. based on the highest degree offered, the level of research activity, and/or focus of study. Some schools may put a higher priority on the ecological spaces and biodiversity of their campuses as a product of what area of study they happen to specialize in. Larger, more research-focused institutions may have more funding overall to devote to biodiversity-boosting programs. Let's take a look at Carnegie classification and species richness:
...
Doctoral/research universities with a broad scope of study generally show higher species biodiversity on their campuses, while biodiversity levels within research universities specialized in a specific area of study are not as high. Baccalaureate level institutions, both general and liberal arts focused, also show higher levels. As expected, institutions specialized in fields not ecology/biology-based show lower biodiversity levels, with medical schools and theological seminaries/other faith-related schools having the lowest richness. Schools of art, music, and design show higher levels of biodiversity compared to the other specialized schools. These types of schools, along with liberal arts undergrad schools, may put more of an emphasis on aesthetically-pleasing and green campuses that contribute to improved bird biodiversity.

#### Accounting for Taxonomic Order and Impervious Surface along with Carnegie Classification

How would the relationship between Carnegie classification and species richness change if we also looked at taxonomic order?
...
Interestingly, when accounting for taxonomic order, associates-level schools show highest order diversity, with baccalaureate schools close behind. Doctoral/research universities, both extensive and intensive, shoot below the highest richness levels. Birds of the Passeriformes order contribute a significant amount to species richness across all institutions types.

What if we considered impervious surface, Carnegie classification, and species richness altogether?
...
When accounting for both impervious surface and Carnegie classification, there doesn't seem to be a strong relationship with species richness. Doctoral/research universities show mid-to-higher areas of impervious surfaces, most likely because these institutions are generally larger.
***
