...
```{r}
#| fig-cap: Facet line plots indicating the relationship between average percentage of imervious surface and bird biodiversity for ech locale
ebirds_ipeds |>
  filter(!is.na(mean_impervious), !is.na(species_count)) |>
  mutate(
    LOCALE = as.factor(LOCALE),
    LOCALE = fct_recode(LOCALE,
                        "City: Large"      = "11",
  ...
  ggplot(aes(x = mean_impervious, y = species_count)) +
 geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs", k = 5), aes(color = LOCALE)) +
  facet_wrap(~ LOCALE) +
  labs(
...
y = "Species Count")
``` 
It seems like across locales, species counts drop below 50 when average impervious surface is above 75%. However, there doesn't seem to be a locale-specific pattern in covariation between the two. 
